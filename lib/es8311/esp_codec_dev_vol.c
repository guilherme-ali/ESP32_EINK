 ...
 * Portado sem alteracoes de esp_codec_dev (ver audio_codec_if.h).
 */
#include <math.h>
...
int esp_codec_dev_vol_calc_reg(const esp_codec_dev_vol_range_t *vol_range, float db) {
  ...
  float ratio = (vol_range->max_vol.vol - vol_range->min_vol.vol) /
                (vol_range->max_vol.db_value - vol_range->min_vol.db_value);
...
float esp_codec_dev_vol_calc_db(const esp_codec_dev_vol_range_t *vol_range, int vol) {
  ...
  if (vol_range->max_vol.vol > vol_range->min_vol.vol) {
    if (vol >= vol_range->max_vol.vol) return vol_range->max_vol.db_value;
    if (vol <= vol_range->min_vol.vol) return vol_range->min_vol.db_value;
  } else {
    if (vol <= vol_range->max_vol.vol) return vol_range->max_vol.db_value;
    if (vol >= vol_range->min_vol.vol) return vol_range->min_vol.db_value;
  }
  float ratio = (vol_range->max_vol.db_value - vol_range->min_vol.db_value) /
                (vol_range->max_vol.vol - vol_range->min_vol.vol);
...
float esp_codec_dev_col_calc_hw_gain(esp_codec_dev_hw_gain_t *hw_gain) {
  ...
  if (pa_voltage == 0.0) pa_voltage = 5.0;
  if (dac_voltage == 0.0) dac_voltage = 3.3;
